...
function objectPreview:create(element, rotX, rotY, rotZ, projPosX, projPosY, projSizeX, projSizeY, isRelative, postGui, isSRT)
    ...
    if isRelative == false then projPosX, projPosY, projSizeX, projSizeY = projPosX / scx, projPosY / scy, projSizeX / scx, projSizeY / scy end
    local elementType = getElementType(element)
    -- outputDebugString('objPrev: Identified element as: '..tostring(elementType))
    if elementType == 'ped' or elementType == 'player' then elementType = 'ped' end
    ...
    new.rotationMatrix = createElementMatrix({0, 0, 0}, new.elementRotation)
    new.positionMatrix = createElementMatrix(new.elementRotationOffsets, {0, 0, 0})
    new.transformMatrix = matrixMultiply(new.positionMatrix, new.rotationMatrix)
    new.multipliedMatrix = matrixMultiply(new.transformMatrix, cameraMatrix)

    local rotx, roty, rotz = getEulerAnglesFromMatrix(new.multipliedMatrix)
    ...
    if elementType == 'vehicle' then
        ...
        for i = 0, 5 do setVehicleDoorState(new.element, i, 0) end
    elseif elementType == 'ped' then
        new.zDistanceSpread = -5
    else
    ...
    if not new.shader then return false end

    if isMRTShaderSupported and glRenderTarget and new.isSecondRT then dxSetShaderValue(new.shader, 'secondRT', glRenderTarget) end

    dxSetShaderValue(new.shader, 'sFov', math.rad(fov))
    dxSetShaderValue(new.shader, 'sAspect', (scy / scx))
    dxSetShaderValue(new.shader, 'sProjZMult', 3)

    local proj = new.projection
    local projSizeX, projSizeY = proj[3] * 0.5, proj[4] * 0.5
    local projPosX, projPosY = proj[1], proj[2]
    projPosX, projPosY = (projPosX + projSizeX - 0.5) * 2, -(projPosY + projSizeY - 0.5) * 2

    dxSetShaderValue(new.shader, 'sMoveObject2D', projPosX, projPosY + 0.1)
    dxSetShaderValue(new.shader, 'sScaleObject2D', projSizeY, projSizeY)
    ...
    new.onPreRender = function() new:update() end
    addEventHandler('onClientPreRender', root, new.onPreRender, true, 'high+5')
    setmetatable(new, objectPreview_mt)
    -- outputDebugString('objPrev: Created ID: '..new.renID..' for: '..tostring(elementType)) 

    if #refOPTable == 1 then initializeEvents() end

    return new
end

function objectPreview:getID() return self.renID end
...
function objectPreview:destroy()
    ...
    self.element = nil
    if #refOPTable == 0 then stopEvents() end
end
...
function objectPreview:update()
    ...
    local cameraMatrix = getElementMatrix(camera)
    ...
    local rotX, rotY, rotZ = self.rotation[1], self.rotation[2], self.rotation[3]

    local velX, velY, velZ = getCamVelocity()
    local vecLen = (velX ^ 2 + velY ^ 2 + velZ ^ 2) ^ 0.5
...
function getCamVelocity()
    if getTickCount() - getLastTick < 300 then return lastCamVelocity[1], lastCamVelocity[2], lastCamVelocity[3] end
...
function objectPreview:saveToFile(filePath)
    ...
    if glRenderTarget then
        local projPosX, projPosY, projSizeX, projSizeY = unpack(self.projection)
...
function objectPreview:drawRenderTarget()
    ...
    if glRenderTarget then
        local projPosX, projPosY, projSizeX, projSizeY, postGui = unpack(self.projection)
        projPosX, projPosY, projSizeX, projSizeY = projPosX * scx, projPosY * scy, projSizeX * scx, projSizeY * scy
        return dxDrawImageSection(projPosX, projPosY, projSizeX, projSizeY, projPosX, projPosY, projSizeX, projSizeY, glRenderTarget, 0, 0, 0,
...
function initializeEvents() --
    if eventsAdded then return end
    addEventHandler('onClientPreRender', root, onPreRender2, true, 'low-5')
    addEventHandler('onClientHUDRender', root, onHUDRender, true, 'low-10')
    eventsAdded = true
end

function stopEvents()
    if not eventsAdded then return end
    removeEventHandler('onClientPreRender', root, onPreRender2)
    removeEventHandler('onClientHUDRender', root, onHUDRender)
    eventsAdded = false
end

-- addEventHandler('onClientResourceStart', resourceRoot, initializeEvents)

function onPreRender2()
    if not isMRTShaderSupported or (#refOPTable == 0) then return end
    if glRenderTarget then
...
function onHUDRender()
    ...
    for index, this in ipairs(refOPTable) do
        ...
        if refOPTable[index] then
            if refOPTable[index].isSecondRT then isMRTUsed = true end
            if refOPTable[index].enabled then
                local instance = this.instance
                if instance then instance:drawRenderTarget() end
            end
        end
    end

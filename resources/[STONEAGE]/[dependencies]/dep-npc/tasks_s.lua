...
function performTask.walkToPos(npc, task, maxtime)
    ...
    if getElementModel(npc) == 0 then return end
...
function performTask.walkAroundBend(npc, task, maxtime)
    if getElementSyncer(npc) then return maxtime end
    return makeNPCWalkAroundBend(npc, task[2], task[3], task[4], task[5], task[6], task[7], task[8])
end
...
function performTask.driveToPos(npc, task, maxtime)
    ...
    return makeNPCDriveToPos(npc, task[2], task[3], task[4])
end
...
function performTask.driveAlongLine(npc, task, maxtime)
    ...
    if not vehicle then return 0 end
    if getElementSyncer(vehicle) then return maxtime end
    return makeNPCDriveAlongLine(npc, task[2], task[3], task[4], task[5], task[6], task[7], task[8])
end
...
function performTask.driveAroundBend(npc, task, maxtime)
    ...
    if not vehicle then return 0 end
    if getElementSyncer(vehicle) then return maxtime end
    return makeNPCDriveAroundBend(npc, task[2], task[3], task[4], task[5], task[6], task[7], task[8], task[9], task[10])
end
